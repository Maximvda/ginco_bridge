...
static bool configure(int sock) {
    ...
    if (mqtt) {
        ...
        if (gotssid) {
            app::taskFinder().network().setSsidPass(
                std::string(ssid.get()),
                std::string(password.get())
            );
        }
        ConfigDriver::instance().setKey(ConfigKey::MQTT_URL, std::string(mqtt.get()));
        ConfigDriver::instance().setKey(ConfigKey::WIFI_CONFIGURED, true);
...
static void terminate(int listen_sock) {
    ...
    app::taskFinder().network().startInterface();
    vTaskDelete(NULL);
    return;
}
